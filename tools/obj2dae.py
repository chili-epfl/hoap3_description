...
./blender.bin --background --python source/obj2dae.py -- \
    --path="/fe/obj" \
    --match="*.obj" \
    --start=0 --end=10 \
    --save_path=/tmp/test
...
def batch_import(operator="",
    ...
    for i, f in enumerate(files):
        print("  import_scene.obj(filepath=%r) # %d of %d" % (f, i + start, len(files)))
        ...
        result = bpy.ops.import_scene.obj(filepath=f,axis_forward='Z', axis_up='-Y')
        ...
        if save_path:
            ...
            fout_dae = os.path.splitext(fout)[0] + ".dae"
            ...
            #bpy.ops.wm.collada_export(filepath=fout_dae)
            bpy.context.scene.collada_export(filepath=fout_dae)
